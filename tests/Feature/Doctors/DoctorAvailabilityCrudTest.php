...
class DoctorAvailabilityCrudTest extends TestCase
{
    ...
    public function test_save_available_time_accepts_mobile_controlled_max_reps_per_range(): void
    {
    ...
    {
        $doctor = $this->createDoctor('doctor-save-visit-time-type@example.com', '01111111262');
        Sanctum::actingAs($doctor, ['doctor']);

        foreach (['before', 'after', 'between'] as $index => $visitTimeType) {
            $hour = 9 + $index;
            $response = $this->putJson('/api/doctor/save-available-time', [
                'date' => ['monday', 'tuesday', 'wednesday'][$index],
                'start_time' => sprintf('%02d:00 AM', $hour),
                'end_time' => sprintf('%02d:30 AM', $hour),
                'visit_time_type' => $visitTimeType,
            ]);

            $response->assertStatus(200);
            $response->assertJsonPath("data.available_times.{$index}.visit_time_type", $visitTimeType);
    ...
    public function test_save_available_time_rejects_invalid_visit_time_type_values(): void
    {
        $doctor = $this->createDoctor('doctor-save-invalid-visit-time-type@example.com', '01111111263');
        Sanctum::actingAs($doctor, ['doctor']);

        foreach (['during', '', 'random'] as $visitTimeType) {
            $response = $this->putJson('/api/doctor/save-available-time', [
                'date' => 'monday',
                'start_time' => '09:00 AM',
                'end_time' => '10:00 AM',
    ...
    public function test_save_available_time_defaults_visit_time_type_to_between_when_omitted(): void
    {
        $doctor = $this->createDoctor('doctor-save-default-visit-time-type@example.com', '01111111264');
        Sanctum::actingAs($doctor, ['doctor']);
        ...
        $response->assertStatus(200);
        $response->assertJsonPath('data.available_times.0.visit_time_type', 'between');
    ...
    public function test_save_available_time_does_not_enforce_duration_based_upper_bound_for_max_reps_per_range(): void
    {
        ...
        $response = $this->putJson('/api/doctor/save-available-time', [
            ...
            'start_time' => '09:00 AM',
            'end_time' => '10:00 AM',
            'max_reps_per_range' => 50,
        ]);
        $response->assertStatus(200);
        $response->assertJsonPath('data.available_times.0.max_reps_per_range', 50);
    ...
    public function test_update_available_time_updates_max_reps_per_range_and_opens_capacity_when_omitted(): void
    {
        ...
        $updateWithOneResponse = $this->putJson('/api/doctor/available-time/' . $availability->id, [
            ...
            'start_time' => '09:00 AM',
            'end_time' => '10:00 AM',
            'max_reps_per_range' => 25,
        ]);
        $updateWithOneResponse->assertStatus(200);
        $updateWithOneResponse->assertJsonPath('data.max_reps_per_range', 25);
        $updateWithOneResponse->assertJsonPath('data.visit_time_type', 'before');
    ...
    public function test_update_available_time_updates_visit_time_type(): void
    {
        $doctor = $this->createDoctor('doctor-update-visit-time-type@example.com', '01111111265');
        $availability = $this->createAvailability($doctor, 'monday', '09:00:00', '10:00:00', 'available', false, 2, 'before');
        Sanctum::actingAs($doctor, ['doctor']);
        ...
        $response->assertStatus(200);
        $response->assertJsonPath('data.visit_time_type', 'after');
    ...
    public function test_representative_doctor_profile_endpoint_returns_only_available_times(): void
    {
        ...
        $this->createAvailability($doctor, 'monday', '09:00:00', '10:00:00', 'available', false, 2, 'after');
        $this->createAvailability($doctor, 'tuesday', '10:00:00', '11:00:00', 'busy');
        ...
        $response->assertJsonPath('data.available_times.0.visit_time_type', 'after');
    }
    ...
    public function test_doctor_profile_returns_availability_booking_counts_and_no_times_booked(): void
    {
        ...
        try {
            ...
            $otherRep = $this->createRepresentative('rep-times-booked-no-date-other@example.com', '01011111209');

            $this->createAppointment($doctor, $rep, $today, '09:00:00', '10:00:00', 'pending', $availability);
            $this->createAppointment($doctor, $otherRep, $today, '09:00:00', '10:00:00', 'confirmed', $availability);
            $this->createAppointment($doctor, $rep, $today, '09:00:00', '10:00:00', 'cancelled', $availability);

            Sanctum::actingAs($rep, ['representative']);

            $profileResponse = $this->getJson('/api/reps/docotr/' . $doctor->id);
            $profileResponse->assertStatus(200);
            $profileResponse->assertJsonPath('data.booked_for_date', null);
            $profileResponse->assertJsonPath('data.available_times.0.counted_for_date', $today);
            $profileResponse->assertJsonPath('data.available_times.0.booked_reps_count', 2);
            $profileResponse->assertJsonPath('data.available_times.0.remaining_reps_count', 0);
            $profileResponse->assertJsonPath('data.available_times.0.is_booked_for_date', true);
            $this->assertArrayNotHasKey('times_booked', $profileResponse->json('data'));
            ...
            $this->assertNull($doctorPayload['booked_for_date']);
            $this->assertSame($today, $doctorPayload['available_times'][0]['counted_for_date']);
            $this->assertSame(2, $doctorPayload['available_times'][0]['booked_reps_count']);
            $this->assertSame(0, $doctorPayload['available_times'][0]['remaining_reps_count']);
            $this->assertTrue($doctorPayload['available_times'][0]['is_booked_for_date']);
            $this->assertArrayNotHasKey('times_booked', $doctorPayload);
        } finally {
            Carbon::setTestNow();
        }
    }

    public function test_availability_booking_counts_are_scoped_to_requested_date(): void
    {
        ...
        try {
            ...
            $availability = $this->createAvailability($doctor, 'monday', '09:00:00', '10:00:00', 'available', false, 3);
            $rep = $this->createRepresentative('rep-times-booked-with-date@example.com', '01011111202');

            $this->createAppointment($doctor, $rep, $beforeAnchor, '09:00:00', '10:00:00', 'pending', $availability);
            $this->createAppointment($doctor, $rep, $atAnchor, '09:00:00', '10:00:00', 'pending', $availability);
            $this->createAppointment($doctor, $rep, $afterAnchor, '09:00:00', '10:00:00', 'confirmed', $availability);
            ...
            $response->assertJsonPath('data.available_times.0.booked_reps_count', 1);
            $response->assertJsonPath('data.available_times.0.remaining_reps_count', 2);
    ...
    public function test_open_capacity_availability_returns_null_remaining_count(): void
    {
        Carbon::setTestNow(Carbon::create(2026, 4, 1, 10, 0, 0, 'Africa/Cairo'));
        ...
        try {
            ...
            $availability = $this->createAvailability($doctor, 'monday', '09:00:00', '10:00:00', 'available', false, null);
            $rep = $this->createRepresentative('rep-times-booked-horizon@example.com', '01011111203');

            $this->createAppointment($doctor, $rep, $anchorDate->toDateString(), '09:00:00', '10:00:00', 'pending', $availability);
            ...
            $response->assertStatus(200);
            $response->assertJsonPath('data.available_times.0.booked_reps_count', 1);
            $response->assertJsonPath('data.available_times.0.remaining_reps_count', null);
            $response->assertJsonPath('data.available_times.0.is_booked_for_date', false);
        } finally {
            Carbon::setTestNow();
        }
    ...
    public function test_no_date_doctors_all_counts_use_next_occurrence_after_booking(): void
    {
        Carbon::setTestNow(Carbon::create(2026, 5, 26, 10, 0, 0, 'Africa/Cairo'));
        ...
        try {
            ...
            $doctor = $this->createDoctor('rep-no-date-listing@example.com', '01111111307');
            ...
            $rep = $this->createRepresentative('rep-no-date-listing@example.com', '01011111208');
            $rep->company()->update(['visits_per_day' => 5]);

            Sanctum::actingAs($rep, ['representative']);

            $this->postJson('/api/reps/booking', $this->bookingPayload($doctor, $bookingDate, $availability))
                ->assertStatus(201);

            $listingResponse = $this->getJson('/api/reps/doctors/all?page=1&per_page=10');
            $listingResponse->assertStatus(200);
            $doctorPayload = collect($listingResponse->json('data'))->firstWhere('id', $doctor->id);
            $this->assertNotNull($doctorPayload);
            $this->assertNull($doctorPayload['booked_for_date']);
            $this->assertSame($bookingDate, $doctorPayload['available_times'][0]['counted_for_date']);
    ...
    public function test_no_date_favorite_doctors_counts_use_next_occurrence_after_booking(): void
    {
        Carbon::setTestNow(Carbon::create(2026, 5, 26, 10, 0, 0, 'Africa/Cairo'));
        ...
        try {
            ...
            $doctor = $this->createDoctor('rep-no-date-favorite@example.com', '01111111308');
            $availability = $this->createAvailability(
                $doctor,
                $bookingWeekday,
                '09:00:00',
            ...
            $rep = $this->createRepresentative('rep-no-date-favorite@example.com', '01011111209');
            $rep->company()->update(['visits_per_day' => 5]);
            ...
            $favoriteResponse = $this->getJson('/api/reps/favorite-doctors');
            ...
            $this->assertNull($favoritePayload['booked_for_date']);
            $this->assertSame($bookingDate, $favoritePayload['available_times'][0]['counted_for_date']);
    ...
    public function test_no_date_listing_returns_zero_remaining_when_next_occurrence_is_full(): void
    {
        Carbon::setTestNow(Carbon::create(2026, 5, 26, 10, 0, 0, 'Africa/Cairo'));
        ...
        try {
            ...
            $doctor = $this->createDoctor('rep-no-date-full@example.com', '01111111309');
            $availability = $this->createAvailability(
                $doctor,
                $bookingWeekday,
                '09:00:00',
            ...
            $firstRep = $this->createRepresentative('rep-no-date-full-a@example.com', '01011111210');
            $secondRep = $this->createRepresentative('rep-no-date-full-b@example.com', '01011111211');
            $firstRep->company()->update(['visits_per_day' => 5]);
            $secondRep->company()->update(['visits_per_day' => 5]);

            $this->createAppointment($doctor, $firstRep, $bookingDate, '09:00:00', '10:00:00', 'pending', $availability);
            $this->createAppointment($doctor, $secondRep, $bookingDate, '09:00:00', '10:00:00', 'confirmed', $availability);
    ...
    public function test_explicit_date_still_overrides_next_occurrence_counter_scope(): void
    {
        Carbon::setTestNow(Carbon::create(2026, 5, 26, 10, 0, 0, 'Africa/Cairo'));
        ...
        try {
            ...
            $doctor = $this->createDoctor('rep-explicit-date-override@example.com', '01111111310');
            $availability = $this->createAvailability(
                $doctor,
                $bookingWeekday,
                '09:00:00',
            ...
            $rep = $this->createRepresentative('rep-explicit-date-override@example.com', '01011111212');
            $rep->company()->update(['visits_per_day' => 5]);
            ...
            $noDateResponse = $this->getJson('/api/reps/docotr/' . $doctor->id);
            $noDateResponse->assertJsonPath('data.available_times.0.booked_reps_count', 1);
            $noDateResponse->assertJsonPath('data.available_times.0.counted_for_date', $bookingDate);

            $explicitResponse = $this->getJson('/api/reps/docotr/' . $doctor->id . '?date=' . $otherOccurrenceDate);
            $explicitResponse->assertStatus(200);
            $explicitResponse->assertJsonPath('data.booked_for_date', $otherOccurrenceDate);
            $explicitResponse->assertJsonPath('data.available_times.0.counted_for_date', $otherOccurrenceDate);
            $explicitResponse->assertJsonPath('data.available_times.0.booked_reps_count', 0);
            $explicitResponse->assertJsonPath('data.available_times.0.remaining_reps_count', 2);
            $explicitResponse->assertJsonPath('data.available_times.0.is_booked_for_date', false);
        } finally {
            Carbon::setTestNow();
        }
    }
    ...
    public function test_is_booked_for_date_remains_tied_to_target_date_even_when_future_bookings_exist(): void
    {
        ...
        try {
            ...
            $availability = $this->createAvailability($doctor, 'monday', '09:00:00', '10:00:00', 'available', false, 1);
            ...
            $this->createAppointment($doctor, $rep, $futureDate, '09:00:00', '10:00:00', 'pending', $availability);
            ...
            $response->assertJsonPath('data.available_times.0.status', 'available');
            $response->assertJsonPath('data.available_times.0.is_booked_for_date', false);
            $response->assertJsonPath('data.available_times.0.booked_reps_count', 0);
            $response->assertJsonPath('data.available_times.0.remaining_reps_count', 1);
        } finally {
            Carbon::setTestNow();
        }
    }

    public function test_overnight_booking_counts_only_for_start_day(): void
    {
        Carbon::setTestNow(Carbon::create(2026, 4, 1, 10, 0, 0, 'Africa/Cairo'));
        ...
        try {
            ...
            $availability = $this->createAvailability($doctor, 'sunday', '22:00:00', '02:00:00', 'available', true, 1);
            ...
            $startDayResponse->assertJsonPath('data.available_times.0.booked_reps_count', 1);
            $startDayResponse->assertJsonPath('data.available_times.0.remaining_reps_count', 0);

            $nextDayResponse = $this->getJson('/api/reps/docotr/' . $doctor->id . '?date=' . $nextDate->toDateString());
            $nextDayResponse->assertStatus(200);
            $nextDayResponse->assertJsonPath('data.available_times.0.is_booked_for_date', false);
            $nextDayResponse->assertJsonPath('data.available_times.0.booked_reps_count', 0);
            $nextDayResponse->assertJsonPath('data.available_times.0.remaining_reps_count', 1);
        } finally {
            Carbon::setTestNow();
        }
    }
    ...
    public function test_representative_doctor_listing_endpoints_return_only_available_times(): void
    {
        ...
        $this->createAvailability($doctor, 'monday', '09:00:00', '10:00:00', 'available', false, 2, 'before');
        $this->createAvailability($doctor, 'tuesday', '10:00:00', '11:00:00', 'busy');
        $this->createAvailability($doctor, 'wednesday', '11:00:00', '12:00:00', 'booked');
        $this->createAvailability($doctor, 'thursday', '12:00:00', '13:00:00', 'canceled');
        ...
        $this->assertSame('before', $doctorPayload['available_times'][0]['visit_time_type']);
        ...
        $this->assertSame('before', $searchPayload['available_times'][0]['visit_time_type']);
        ...
        $this->assertSame('before', $specialityPayload['available_times'][0]['visit_time_type']);
    }
    ...
    {
        $busyDate = Carbon::now('Africa/Cairo')->addDay()->toDateString();
        $outsideDate = Carbon::now('Africa/Cairo')->addDays(2)->toDateString();
        ...
        $busyAvailability = $this->createAvailability(
            $doctor,
            strtolower(Carbon::parse($busyDate)->format('l')),
            '09:00:00',
            '11:00:00',
            'available'
        );
        $allowedAvailability = $this->createAvailability(
            $doctor,
        ...
        $blockedResponse = $this->postJson('/api/reps/booking', $this->bookingPayload($doctor, $busyDate, $busyAvailability));
        $blockedResponse->assertStatus(403);
        $blockedResponse->assertJsonPath('message', 'Doctor is busy during the selected period');

        $allowedResponse = $this->postJson('/api/reps/booking', $this->bookingPayload($doctor, $outsideDate, $allowedAvailability));
    ...
    public function test_representative_booking_skips_range_capacity_when_max_reps_per_range_is_open(): void
    {
        ...
        foreach ([0, 1, 2] as $index) {
            ...
            $response = $this->postJson('/api/reps/booking', $this->bookingPayload($doctor, $bookingDate, $availability));
    ...
    public function test_doctor_update_available_time_with_same_weekday_non_overlapping_rows_keeps_both_available(): void
    {
        ...
        $anotherAvailability = $this->createAvailability($doctor, 'thursday', '10:30:00', '11:30:00');
    ...
    {
        $appointmentDate = Carbon::now('Africa/Cairo')->addDays(14)->toDateString();
        $doctor = $this->createDoctor('doctor-update-appointment-overlap@example.com', '01111111213');
        $availability = $this->createAvailability($doctor, $appointmentDate, '09:00:00', '10:00:00');
    ...
    {
        $startDate = Carbon::now('Africa/Cairo')->addDays(14)->toDateString();
        $nextDate = Carbon::parse($startDate, 'Africa/Cairo')->addDay()->toDateString();
        $doctor = $this->createDoctor('doctor-update-overnight-overlap@example.com', '01111111217');
        $availability = $this->createAvailability($doctor, $startDate, '18:00:00', '20:00:00');
    ...
    private function bookingPayload(Doctors $doctor, string $date, DoctorAvailability $availability): array
    {
        return [
            'doctors_id' => $doctor->id,
            'date' => $date,
